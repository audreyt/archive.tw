function cw2pad(x){
  ...
  for (i$ = 0, len$ = ref$.length; i$ < len$; ++i$) {
    ...
    if (/(?:[A-Z]\w*\.)*[-'ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿ\w ]+:(?!\d)/.exec(ln) && /^((?:[A-Z]\w*\.)*(?:\s\w\.|[-'ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿ\w ]+)+):(?!\d)/.exec(ln)) {
      var n = RegExp.$1;
      if (/ /.test(n)) {
        names[n.replace(/ .*/, '')] = n;
      }
